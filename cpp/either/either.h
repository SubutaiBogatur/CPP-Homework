...
struct either
{
    ...
    either& operator=(either); //inside always move

    //accessing
    bool is_left() const;
    Left& left();
    Left const& left() const; //if Right currently stored, UB

    bool is_right() const;
    Right& right();
    Right const& right() const; //if Left currently stored, UB

    //emplace functions satisfy strong exception safety guarantee! (on some conditions)
    //they are implemented using double buffering technique.
    // ie firstly space in heap is allocated and copy of current object in data is created, using copy constructor, in heap
    //  (if copy constructor throws exception, old value is not lost, everything ok)
    // then state of either is changed to pointer to heap and current value in data is destroyed (suppose destructor no throw)
    // then we are trying to construct new object in data using given arguments
    // if constructor fails, unique pointer to heap, where old object was saved, is created, using copy constructor,
    //  in data (suppose not fails). Everything ok, strong safety provided.
    // if constructor succeeds, everything is obvious.
    //same technique is used in swap implementation

    //moves either to left condition, left is created with constructor from these arguments
    template<typename... Args>
    void emplace(emplace_left_t, Args&& ... args);

    //moves either to right condition, right is created with constructor from these args
    template<typename... Args>
...
void swap(either<Left, Right>& lhs, either<Left, Right>& rhs)
{
    if (lhs.is_left() && rhs.is_left())
        ...
        swap_impl(lhs, rhs, std::unique_ptr<Left>(std::make_unique<Left>(lhs.left())),
